...
#add title, color, shape, and size of shape

ggplot(data = hippocampus, mapping = aes(x=posterior_hippo_vol, y=months_as_taxi_driver))+geom_point(shape=11, size=3, color="red")+ggtitle("Hippocampus data")+theme(panel.background = element_rect(fill = 'green'))
...
#Input as a vectorusing c() function
...
#
#ggplot(data = mass2 )+geom_histogram(aes(mass2$mass))


ggplot(data = mass2,aes(mass2$mass))+geom_histogram(breaks=seq(0.037,0.172,0.001))
